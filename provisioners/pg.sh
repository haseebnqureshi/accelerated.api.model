...
echo "[accelerated.api.model/provisioners/pg.sh] Installing postgres ..."

apt-get install postgresql postgresql-contrib -y > /dev/null
apt-get install postgresql-contrib libpq-dev -y > /dev/null

echo "[accelerated.api.model/provisioners/pg.sh] -- Installing nodejs postgres drivers"

npm install pg --save --loglevel=error > /dev/null
npm install pg-native --save --loglevel=error > /dev/null

echo "[accelerated.api.model/provisioners/pg.sh] -- Creating database user and granting privileges"
...
echo "[accelerated.api.model/provisioners/pg.sh] -- Configuring postgres"
...
echo "[accelerated.api.model/provisioners/pg.sh] -- Restarting postgres service"

su postgres service postgresql restart

echo "[accelerated.api.model/provisioners/pg.sh] -- Creating & importing database $DB_NAME"

su postgres -c "psql -c \"CREATE EXTENSION pgcrypto;\" -d $DB_NAME"
su postgres -c "createdb $DB_NAME"
su postgres -c "psql -f $DB_NAME.sql -d $DB_NAME"

echo "[accelerated.api.model/provisioners/pg.sh] Finished installing postgres!"
