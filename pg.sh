...
echo "[accelerated.api.model/pg.sh] -- Configuring postgres"
...
echo "[accelerated.api.model/pg.sh] -- Restarting postgres service"

su postgres service postgresql restart

echo "[accelerated.api.model/pg.sh] -- Creating & importing database $DB_NAME"

su postgres -c "psql -c \"CREATE EXTENSION pgcrypto;\" -d $DB_NAME"
su postgres -c "createdb $DB_NAME"
su postgres -c "psql -f $DB_NAME.sql -d $DB_NAME"

echo "[accelerated.api.model/pg.sh] Finished installing postgres!"
